...
func makeClientHelloGM(config *Config) (*clientHelloMsg, error) {
	...
	if config.ForceMix {
		if hello.cipherSuites == nil {
			hello.cipherSuites = defaultCipherSuites()
			hello.cipherSuites = append(hello.cipherSuites, GMTLS_SM2_WITH_SM4_SM3)
			hello.cipherSuites = append(hello.cipherSuites, GMTLS_ECDHE_SM2_WITH_SM4_SM3)
		}
		hello.supportedCurves = []CurveID{23, 24, 25, 29}
...
func (hs *clientHandshakeStateGM) handshake() error {
	...
	if !hs.forceMix && hs.serverHello.vers != VersionGMSSL {
...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if !hs.forceMix && len(certMsg.certificates) < 2 {
	...
	if c.config.ForceMix {
		cert = c.peerCertificates[0]
	} else {
		cert = c.peerCertificates[1]
	}
...
func (hs *clientHandshakeStateGM) getCertificate(certReq *certificateRequestMsg) (*Certificate, error) {
	c := hs.c
