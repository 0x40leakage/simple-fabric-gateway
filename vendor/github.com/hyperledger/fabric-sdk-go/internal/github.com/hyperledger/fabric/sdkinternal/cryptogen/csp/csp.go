...
func LoadPrivateKey(keystorePath string, bccspType string) (bccsp.Key, crypto.Signer, error) {
	var err error
	var priv bccsp.Key
	var s crypto.Signer

	opts := &factory.FactoryOpts{
		ProviderName: bccspType,
	}
	if bccspType == "GM" {
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		if strings.HasSuffix(path, "_sk") {
			...
			s, err = signer.New(wrapper.NewCryptoSuite(csp), wrapper.GetKey(priv))
			if err != nil {
				return err
			}
...
func GeneratePrivateKey(keystorePath string, bccspType string) (bccsp.Key,
	crypto.Signer, error) {
	var err error
	var priv bccsp.Key
	var s crypto.Signer
