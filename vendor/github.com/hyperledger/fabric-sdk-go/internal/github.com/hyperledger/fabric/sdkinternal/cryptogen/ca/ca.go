...
func NewCA(
	...
	priv, signer, err := csp.GeneratePrivateKey(baseDir, bccspType)
	if err != nil {
		return nil, err
	}
	pub, err := csp.GetECPublicKey(priv)
	if err != nil {
		return nil, err
	}
...
func (ca *CA) SignCertificate(
	...
	if pubKey, ok := pub.(bccsp.Key); ok {
		template.SubjectKeyId = pubKey.SKI()
	}
	//set the organization for the subject
