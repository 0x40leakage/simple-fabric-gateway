...
import (
	...
	fabricmsp "github.com/hyperledger/fabric-sdk-go/internal/github.com/hyperledger/fabric/msp"
...
func GenerateLocalMSP(baseDir, name string, sans []string, signCA *ca.CA,
	nodeType int, nodeOUs bool, bccspType string) error {
	...
	*/
	...
	priv, _, err := csp.GeneratePrivateKey(keystore, bccspType)
	if err != nil {
		return err
	}
	...
	cert, err := signCA.SignCertificate(filepath.Join(mspDir, "signcerts"),
		name, ous, nil, ecPubKey, x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{}, bccspType)
	if err != nil {
		return err
	}
...
func GenerateLocalTLS(baseDir, name string, sans []string, tlsCA *ca.CA,
	nodeType int, nodeOUs bool, bccspType string) (err error) {
	/*
	...
	*/
	...
	err = os.MkdirAll(filepath.Join(mspDir, "tlscacerts"), 0755)
	if err != nil {
		return err
	}
	// the TLS CA certificate goes into tlscacerts
	...
	//// generate private key
	tlsPrivKey, _, err := csp.GeneratePrivateKey(tlsDir, bccspType)
	if err != nil {
		return err
	}
	// get public key
	...
	_, err = tlsCA.SignCertificate(filepath.Join(tlsDir),
		...
		[]x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth, x509.ExtKeyUsageClientAuth}, bccspType)
	if err != nil {
		return err
	}
	...
	if bccspType == "SW" || nodeType == CLIENT || nodeType == ADMIN {
		return
	}
	//// generate enc private key
	tlsEncPrivKey, _, err := csp.GeneratePrivateKey(tlsDir, bccspType)
	if err != nil {
		return err
	}
	// get public key
	tlsEncPubKey, err := csp.GetECPublicKey(tlsEncPrivKey)
	if err != nil {
		return err
	}
	// generate X509 certificate using TLS CA
	_, err = tlsCA.SignCertificate(filepath.Join(tlsDir),
		name, nil, sans, tlsEncPubKey, x509.KeyUsageKeyEncipherment|x509.KeyUsageDataEncipherment|x509.KeyUsageKeyAgreement,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth, x509.ExtKeyUsageClientAuth}, bccspType)
	if err != nil {
		return err
	}
	...
	// rename the generated TLS X509 cert
	tlsEncFilePrefix := "serverenc"
	err = os.Rename(filepath.Join(tlsDir, x509Filename(name)),
		filepath.Join(tlsDir, tlsEncFilePrefix+".crt"))
	if err != nil {
		return err
	}

	err = keyExport(tlsDir, filepath.Join(tlsDir, tlsEncFilePrefix+".key"), tlsEncPrivKey)
	if err != nil {
		return err
	}
	return
...
func GenerateVerifyingMSP(baseDir string, signCA *ca.CA, nodeOUs bool, bccspType string) error {
	...
	_, err = signCA.SignCertificate(filepath.Join(baseDir, "admincerts"), signCA.Name,
		nil, nil, ecPubKey, x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{}, bccspType)
	if err != nil {
		return err
	}
	return nil
