...
func (c *fabricCAAdapter) GenCRL(key core.Key, cert []byte, request *caapi.GenCRLRequest) (*api.GenCRLResponse, error) {
	//logger.Debugf("Entering identity.GenCRL %+v", req)
	registrar, err := c.newIdentity(key, cert)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create CA signing identity")
	}

	response, err := registrar.GenCRL(request)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get identity")
	}

	return &api.GenCRLResponse{CRL: response.CRL}, nil
}
